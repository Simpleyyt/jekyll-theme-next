...
`fd.c`는 리눅스 파일 디스크립터와 관련된 문제이다. 위 소스에서 argv의 값이 2 미만일 경우 에러를 출력하는데 이는 0, 1, 2는 이미 예약되어있는데, 각각의 의미는 아래와 같다.

0	|	표준입력
1	|	표준출력
2	|	표준에러

파일 디스크립터는 파일이나 소켓을 관리하기해 운영체제가 필요로 하는 정보를 담고 있는 것이다.
