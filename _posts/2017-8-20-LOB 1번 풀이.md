...
처음부터 LOB를 풀자니 조금 어려워서, 비슷한 오버플로우 문제인 FTZ 9번 문제의 write-up을 참고해 문제를 풀었습니다.
...
gdb를 활용해 gremlin의 main함수를 뜯어보면 `main+3` 에서 0x100, 즉 256바이트만큼 ESP의 값을 낮춤으로써 버퍼를 확보하는것이 보입니다. 추가적인 더미 메모리 구조는 보이지 않으므로 메모리 구조는
[ argv(4byte) - argc(4byte) - ret(4byte) - sfp(4byte) - buffer(256byte) ] 가 됩니다.
...
gremlin 파일에는 setuid가 걸려있으므로 buffer를 넘어서서 sfp 값을 덮어쓰고, ret값을 수정하면 root권한의 쉘을 실행시킬 수 있겠습니다.